...
- (NSMutableArray *)getReginPlistData
{
    ...
    [regiondic enumerateKeysAndObjectsUsingBlock:^(NSString * firstkey, NSString * firstTitle, BOOL * _Nonnull stop) {
        ...
        if (![firstkey isEqualToString:@"sort"])
        {
            ...
            NSString * FirstSelectName = [[NSUserDefaults standardUserDefaults]objectForKey:@"FirstListSelect"];
            
            if (FirstSelectName.length > 0)
            {
                if ([FirstSelectName isEqualToString:DEFAULTREGINNAME])
                {
